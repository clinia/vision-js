...
`);
    ...
    describe('with a started instance', () => {
      ...
      it('schedules a search to take the added widgets into account', () => {
        ...
        expect(visionInstance.scheduleSearch).toHaveBeenCalledTimes(0);

        instance.addWidgets([createSearchBox()]);

        expect(visionInstance.scheduleSearch).toHaveBeenCalledTimes(1);
      });
...
`);
    ...
    describe('with a started instance', () => {
      ...
      it('updates the local `uiState` with removed widgets', () => {
        ...
        // `visionInstance` must have been notified 2 times of the `uiState` changes:
        ...
        expect(visionInstance.onStateChange).toHaveBeenCalledTimes(2);
      });
  ...
  describe('init', () => {
    ...
    it('uses the internal state for the queries', () => {
      ...
      const mainHelper = searchHelper(searchClient, '', {});
      const visionInstance = createVision({
        mainHelper,
      });
    ...
    it('schedules a render on DerivedHelper results', async () => {
      ...
      expect(visionInstance.scheduleRender).toHaveBeenCalledTimes(0);

      await runAllMicroTasks();

      expect(visionInstance.scheduleRender).toHaveBeenCalledTimes(1);
    });
    ...
    it('schedules a stalled render on DerivedHelper search', () => {
      ...
      expect(visionInstance.scheduleStalledRender).toHaveBeenCalledTimes(0);

      // Simulate a call to search from a widget
      instance.getHelper()!.search();

      expect(visionInstance.scheduleStalledRender).toHaveBeenCalledTimes(1);
    });
    ...
    describe('with uiState', () => {
      ...
      it('does not update the local `uiState` on state changes in `init`', () => {
        ...
        expect(visionInstance.onStateChange).not.toHaveBeenCalled();
      });
      ...
      it('updates the local `uiState` when they differ on first render', () => {
        ...
        expect(visionInstance.onStateChange).toHaveBeenCalledTimes(1);
        expect(instance.getWidgetState({})).toEqual({
          indexName: {
            query: 'Apple iPhone',
          },
      ...
      it('does not update the local `uiState` on first render for children indices', async () => {
        ...
        subLevelInstance
          ...
          // the DerivedHelper does not contain the results. The `lastResults` property
