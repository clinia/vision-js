 ...
 * @requirements
 *
 * Note that the GeoSearch widget uses the [geosearch](https://www.clinia.com/doc/guides/searching/geo-search) capabilities of Clinia. Your records **must** have a `geoPoint` attribute in order to be displayed on the map.
 *
 * Currently, the feature is not compatible with multiple values in the geoPoint attribute.
 *
 * You are also responsible for loading the Google Maps library, it's not shipped with Vision. You need to load the Google Maps library and pass a reference to the widget. You can find more information about how to install the library in [the Google Maps documentation](https://developers.google.com/maps/documentation/javascript/tutorial).
 *
