...
jest.mock('@clinia/search-helper', () => {
  const module = require.requireActual('@clinia/search-helper');
...
describe('Usage', () => {
  it('throws without indexName', () => {
    expect(() => {
      ...
      new Vision({ indexName: undefined });
  ...
  it('throws without searchClient', () => {
    expect(() => {
      // eslint-disable-next-line no-new
      new Vision({ indexName: 'indexName', searchClient: undefined });
    }).toThrowErrorMatchingSnapshot();
  });
  ...
  describe('when insights client is detected', () => {
    const warningMessage = `[VisionJS]: Vision detected the Insights client in the global scope.
...
See documentation: https://www.clinia.com/doc/api-reference/widgets/vision/js/`;
    ...
    it('throws a warning if insightsClient was not passed', () => {
      ...
      const CliniaAnalyticsObject = 'aa';
      global.CliniaAnalyticsObject = CliniaAnalyticsObject;
      global[CliniaAnalyticsObject] = jest.fn();
      ...
      delete global.CliniaAnalyticsObject;
      delete global[CliniaAnalyticsObject];
    });
...
describe('Vision', () => {
  it('calls addCliniaAgent', () => {
    ...
    expect(searchClient.addCliniaAgent).toHaveBeenCalledTimes(1);
    expect(searchClient.addCliniaAgent).toHaveBeenCalledWith(
      `visionjs (${version})`
    );
  });

  it('does not call searchHelper', () => {
    // eslint-disable-next-line no-new
    new Vision({
      indexName: 'indexName',
      searchClient: createSearchClient(),
    ...
    expect(searchHelper).not.toHaveBeenCalled();
  });
...
describe('start', () => {
  it('creates two Helper one for the instance + one for the index', () => {
    ...
    expect(searchHelper).toHaveBeenCalledTimes(0);

    search.start();

    expect(searchHelper).toHaveBeenCalledTimes(2);
    expect(searchHelper).toHaveBeenCalledWith(searchClient, indexName);
  });
...
describe('createURL', () => {
  ...
  it('at top-level returns the default URL for the main index state', () => {
    ...
    router.createURL.mockImplementation(() => 'https://clinia.com');
    ...
    expect(search.createURL()).toBe('https://clinia.com');
  });
...
describe('use', () => {
  it('hooks middleware into the lifecycle before the instance starts', () => {
    ...
    expect(middleware).toHaveBeenCalledWith({ visionInstance: search });
...
describe('UI state', () => {
  it('warns if UI state contains unmounted widgets in development mode', () => {
    ...
    expect(() => {
      ...
      .toWarnDev(`[VisionJS]: The UI state for the index "indexName" is not consistent with the widgets mounted.
