 ...
 * @property {Object[]} items The matched records from Clinia API.
 ...
 * @property {function(): boolean} hasMapMovedSinceLastRefine Return true if the map has move since the last refinement.
 ...
 * @requirements
 *
 * Note that the GeoSearch connector uses the [geosearch](https://www.clinia.com/doc/guides/searching/geo-search) capabilities of Clinia. Your records **must** have a `geoPoint` attribute in order to be passed to the rendering function.
 *
 * Currently, the feature is not compatible with multiple values in the geoPoint attribute.
 *
 ...
 *
 *   markers = items.map(({ geoPoint }) =>
 *     L.marker([geoPoint.lat, geoPoint.lng]).addTo(map)
 *   );
 *
 *   if (markers.length) {
 ...
 * const customGeoSearch = vision.connectors.connectGeoSearch(renderFn);
 *
...
const connectGeoSearch = (renderFn, unmountFn = noop) => {
  ...
  return (widgetParams = {}) => {
    ...
    const init = initArgs => {
      const { state, helper, visionInstance } = initArgs;
    ...
    const render = renderArgs => {
      const { results, helper, visionInstance } = renderArgs;
      ...
      const items = transformItems(
        results.records.filter(record => record.geoPoint)
      );
