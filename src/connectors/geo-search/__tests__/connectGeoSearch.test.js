...
describe('connectGeoSearch', () => {
  ...
  it('expect to render twice during init and render', () => {
    ...
    const visionInstance = { client, helper };
      ...
      results: new SearchResults(helper.state, [
        {
          meta: {},
          records: [
            { id: 123, geoPoint: { lat: 10, lng: 12 } },
            { id: 456, geoPoint: { lat: 12, lng: 14 } },
            { id: 789, geoPoint: { lat: 14, lng: 16 } },
          ],
        },
      ]),
      helper,
  ...
  it('expect to render with enableRefineOnMapMove disabled', () => {
    ...
    widget.render({
      results: new SearchResults(helper.state, [
        {
          meta: {},
          records: [{ id: 123, geoPoint: { lat: 10, lng: 12 } }],
        },
      ]),
      helper,
    });
  ...
  it('expect to render with only geoloc records', () => {
    const render = jest.fn();
    const unmount = jest.fn();
  ...
  it('expect to render with transformed records', () => {
    const render = jest.fn();
    const unmount = jest.fn();
    ...
    expect(render).toHaveBeenLastCalledWith(
      expect.objectContaining({
        items: [
          { id: 123, geoPoint: { lat: 20, lng: 20 } },
          { id: 789, geoPoint: { lat: 20, lng: 20 } },
        ],
      }),
      false
    );
