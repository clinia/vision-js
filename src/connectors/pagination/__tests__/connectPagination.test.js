...
describe('connectPagination', () => {
  ...
  it('connectPagination - Renders during init and render', () => {
...

    {
      ...
      expect(renderOptions.total).toBe(0);
      expect(renderOptions.numPages).toBe(0);
...

    {
      ...
      const renderOptions =
        renderFn.mock.calls[renderFn.mock.calls.length - 1][0];
      expect(renderOptions.currentRefinement).toBe(0);
      expect(renderOptions.total).toBe(1);
      expect(renderOptions.numPages).toBe(1);
    }
  });
  ...
  it('Provides the pages to render (default)', () => {
    ...
    // page 0
    widget.render({
      results: new SearchResults(helper.state, [{ meta: { numPages: 50 } }]),
      state: helper.state,
      helper,
      createURL: () => '#',
    });
