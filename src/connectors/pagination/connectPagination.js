 ...
 * @property {number} total The number of records computed for the last query (can be approximated).
 * @property {number} numPages The number of pages for the result set.
 ...
 *
 * When using the pagination with Clinia, you should be aware that the engine won't provide you pages
 * beyond the 1000th records by default. You can find more information on the [Clinia documentation](https://www.clinia.com/doc/guides/searching/pagination/#pagination-limitations).
 *
 * @type {Connector}
 ...
 * var customPagination = vision.connectors.connectPagination(renderFn);
 *
...
export default function connectPagination(renderFn, unmountFn = noop) {
  ...
  return (widgetParams = {}) => {
    ...
    return {
      ...
      init({ helper, createURL, visionInstance }) {
        this.refine = page => {
          helper.setPage(page);
          helper.search();
        };
      ...
      getMaxPage({ numPages }) {
        return totalPages !== undefined
          ? Math.min(totalPages, numPages)
          : numPages;
      },

      render({ results, state, visionInstance }) {
        const page = state.page || 0;
        const numPages = this.getMaxPage(results);
        pager.currentPage = page;
        pager.total = numPages;
