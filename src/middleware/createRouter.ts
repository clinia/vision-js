  ...
  return ({ visionInstance }) => {
    function topLevelCreateURL(nextState: UiState) {
      const uiState: UiState = Object.keys(nextState).reduce(
        ...
        visionInstance.mainIndex.getWidgetState({})
      );
    ...
    visionInstance._createURL = topLevelCreateURL;
    visionInstance._initialUiState = {
      ...visionInstance._initialUiState,
    ...
    return {
      ...
      subscribe() {
        router.onUpdate(route => {
          ...
          walk(visionInstance.mainIndex, current => {
