...
const withRecordsAndConfigure = (fn, options) =>
  withRecords(
    args => {
      const { search, vision } = args;
...
stories.add(
  'default',
  withRecordsAndConfigure(({ search, container, vision }) =>
    injectGoogleMaps(() => {
      search.addWidgets([
...
//   withHitsAndConfigure(({ search, container, vision }) =>
//     injectGoogleMaps(() => {
...
stories
  ...
  .add(
    'with Records communication (custom)',
    withRecordsAndConfigure(({ search, container, vision }) =>
      injectGoogleMaps(() => {
        const containerElement = document.querySelector(
          '#results-records-container'
        );

        const removeActiveHitClassNames = () => {
          document.querySelectorAll('.record').forEach(el => {
            el.classList.remove('record--active');
          });
        };
        ...
        containerElement.addEventListener('mouseover', event => {
          ...
          if (recordElement) {
            ...
            const id = parseInt(recordElement.id.substr(4), 10);
            const selector = `.my-custom-marker[data-id="${id}"]`;
        ...
        search.addWidgets([
          vision.widgets.geoSearch({
            ...
            customHTMLMarker: {
              ...
              events: {
                mouseover: ({ item }) => {
                  ...
                  const record = document.getElementById(`record-${item.id}`);
            ...
            templates: {
              HTMLMarker: `
                <div class="my-custom-marker" data-id="{{id}}">
                  {{price_formatted}}
                </div>
              `,
            },
  ...
  .add(
    ...
    withRecordsAndConfigure(
      ({ search, container, vision }) =>
        injectGoogleMaps(() => {
          search.addWidgets([
            vision.widgets.geoSearch({
              googleReference: window.google,
